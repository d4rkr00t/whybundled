...
const getLocation = (fullPath, name) => {
...
const joinModules = modules =>
  modules.reduce((acc, module) => {
    if (module.type === "file") {
      ...
      const reasons = module.reasons.reduce((acc, reason) => {
        const clearName = getModuleName(reason.moduleName);
        const location = clearName ? getLocation(reason.module, clearName) : "";
...
module.exports = function analyze(stats, ignore) {
  ...
  return postProcessModules(joinModules(modules));
};

...
module.exports = function print(report, flags) {
  toArray(report)
    .sort((a, b) => b.imported - a.imported)
    .filter(module => module.imported > 0)
    .forEach(module => {
      if (module.type === "file" && module.imported > 0 && !flags.modulesOnly) {
        console.log(printFile(module).join("\n"));
        console.log(chalk.dim("--------------------"));
        console.log();
      } else if (module.type === "module") {
        if (flags.directOnly && module.depsType !== "direct") return;
        if (flags.transitiveOnly && module.depsType !== "transitive") return;
